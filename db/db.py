...
class Database(object):
    ...
    def create_new_table(self, type_code: str) -> Optional[dict]:
        query = 'CREATE TABLE %s {}'
        with _rollback(self.conn) as _db:
            _db.execute(query)

    def insert_into(self, sql: str, data_list: any):
        query = 'INSERT INTO %s;' % sql
        with _rollback(self.conn) as _db:
            _db.execute(query, data_list)
